...
@section('title', 'Candidature - ' . $application->user->name)

@section('content')

    <div class="mb-8">
        <a href="{{ route('admin.applications.index') }}" class="text-xs font-bold text-slate-500 hover:text-red-600 transition">&larr; Retour aux candidatures</a>
        <h1 class="text-2xl font-black text-[#0a1033] mt-2">Candidature de {{ $application->user->name }}</h1>
        <p class="text-sm text-slate-500">Pour : {{ $application->scholarship->title }}</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-3xl border border-gray-200 p-6">
                ...
                @php $profile = $application->user->studentProfile; @endphp
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div><span class="text-slate-400 text-xs block">Nom complet</span><span class="font-semibold text-[#0a1033]">{{ $application->user->name }}</span></div>
                    <div><span class="text-slate-400 text-xs block">Email</span><span class="font-semibold text-[#0a1033]">{{ $application->user->email }}</span></div>
                    ...
                    <div class="col-span-2"><span class="text-slate-400 text-xs block">Adresse</span><span class="font-semibold text-[#0a1033]">{{ $profile->address ?? '—' }}</span></div>
                </div>
            </div>
            ...
            @if($application->motivation)
                <div class="bg-white rounded-3xl border border-gray-200 p-6">
                    <h2 class="text-sm font-bold text-[#0a1033] uppercase tracking-wider mb-4">Lettre de motivation</h2>
                    <p class="text-sm text-slate-600 leading-relaxed whitespace-pre-line">{{ $application->motivation }}</p>
                </div>
            @endif

            <!-- DOCUMENTS -->
            <div class="bg-white rounded-3xl border border-gray-200 p-6">
                <h2 class="text-sm font-bold text-[#0a1033] uppercase tracking-wider mb-4">Documents joints ({{ $application->documents->count() }})</h2>
                <div class="space-y-2">
        ...
        <div>
            <div class="bg-white rounded-3xl border border-gray-200 p-6 sticky top-6">
                <h2 class="text-sm font-bold text-[#0a1033] uppercase tracking-wider mb-4">Statut de la candidature</h2>

                @if($application->scholarship->capacity !== null)
                ...
                <div class="mt-6 pt-6 border-t border-gray-100 text-xs text-slate-400 space-y-1">
                    <p>Candidature reçue le {{ $application->created_at->format('d/m/Y à H:i') }}</p>
                </div>
            </div>
        </div>
